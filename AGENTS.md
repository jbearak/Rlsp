...
- **Path resolution**
  ...
  - **Implicit testthat/testit working directory is a third, SOFT WD tier** (issue #638): files under `<root>/tests/testthat|testit/` anchor forward-relative paths at that directory, but it is deliberately excluded from `cd_in_effect()` — it never suppresses the workspace-root fallback and never propagates into sourced children — is complemented by a file-directory compatibility fallback for nested files, and is overridden by any explicit/inherited `# raven: cd`. Populated only by forward constructors (`PathContext::from_metadata` / `forward_without_metadata`), never `PathContext::new`. Mechanics live in the `implicit_test_working_directory` field/fn doc comments in `path_resolve.rs`.
...
- **Cross-file `# raven: nse` / `# raven: func` propagation**
  - Declarations propagate over the source graph using the **revalidation-consistent set** `S(Q) = ancestors(Q) ∪ descendants(ancestors(Q) ∪ {Q})`. The single shared primitive `DependencyGraph::revalidation_consistent_set` (`cross_file/dependency.rs`) backs both collection (`collect_cross_file_nse` in `handlers.rs`, over the trimmed snapshot subgraph) and revalidation (`compute_affected_dependents_after_edit` in `cross_file/revalidation.rs`, over the full graph), so they cannot drift in edge-selection logic. For an untruncated snapshot neighborhood the trimmed-vs-full asymmetry is intentional and safe-direction (`S_trimmed ⊆ S_full`); if either the depth or visited bound truncates the neighborhood, foreign NSE/func collection fails closed while own-file declarations/directives remain active, because root-dependent budget spending can otherwise break the per-member inverse guarantee. The traversal shape, truncation contract, and per-caller root handling live in those three functions' doc comments.
  ...
  - `non_lending` edges are full-graph revalidation-visible but filtered from trimmed backward indexes and lending consumers, preserving `S_trimmed ⊆ S_full` while preventing excluded buffers from lending scope/declarations.
