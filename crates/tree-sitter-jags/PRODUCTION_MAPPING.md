...
|---|---|---|---|
| `program` | `var` → optional `data` → required `model`; order and multiplicity contrasts | none | ordered top-level block pattern |
...
| `call`, `call_arguments` | nested calls; at least one positional argument; named/missing/trailing controls reject | compatible call/argument fields | none |
| `subset`, `subset_arguments` | empty and omitted dimensions, nesting | compatible subset/argument fields | none |
| unary/binary/parenthesized expressions | operator-by-operator accepted/rejected matrix | precedence implementation idiom | comparison with a block language |
| `number`, `identifier` | decimal/exponent and ASCII-name contrasts | token organization | ASCII identifier restriction |
| `comment` | `#` and `/* */` accept; `//` rejects | `#` token pattern | C-style block-comment token pattern |

The grammar has no external scanner. All 22 named productions and all 35
