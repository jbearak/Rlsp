...
"Under `R/`" means what R loads: the files directly in `R/` plus the two OS-specific subdirectories `R/unix/` and `R/windows/`. R never recurses into `R/` (`R CMD INSTALL`, `devtools::load_all()`, and `pkgload` all use the non-recursive `tools::list_files_with_type()` listing), so a script kept in another subdirectory such as `R/scripts/run.R` is not package source. Raven treats it as an ordinary script: it neither contributes symbols to the namespace nor sees package internals unqualified, and it gets the usual cross-file behavior (`source()` detection, directives) instead.
