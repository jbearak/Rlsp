...
For `x <- y |>` the first operand begins at column 5, so aligned mode requires the continuation under `y`; indented mode requires column 2. The producer follows its own `raven.indentation.infixContinuationStyle` faithfully. Matching producer/lint values never conflict, and lint `either` is compatible with either producer value. Mismatched strict values are a user configuration state. If a flagged non-assignment continuation is at exactly the column the enabled auto-indenter produces, Raven appends advice naming both settings: match the lint style to the producer, use lint `either`, or change the producer to the lint style. No advice is added for assignments, genuinely mis-indented code, disabled/off producer policy, lint `either`, CLI runs without an `[indentation]` policy, or documents whose editor inserts tabs (`editor.insertSpaces` off, synced per document from VS Code) — the auto-indenter stands down in tabs-mode editors, so the flagged column cannot have come from it.
