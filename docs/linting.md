...
- `raven.linting.infixContinuationStyle` (Raven-specific; no `lintr` or `.lintr` equivalent, so a discovered `.lintr` always uses the default) controls how a line continuing an end-of-line infix operator (`|`, `&`, `+`, comparisons, pipes, `%…%`, `$`/`@` chains) is judged:
  - `"indented"` (default) — require the extra continuation level described above, matching `lintr`.
  - `"aligned"` — require the continuation exactly at the operator chain's starting column, so peer operands line up: with a 4-space unit, `changed <- !(\n    first_condition |\n    second_condition\n)` is clean and the extra-indent form is flagged. This is a strict requirement, not a formatter-parity mode: the on-type formatter indents a continuation to at least one unit, so for a chain starting at a line's first column (e.g. a top-level `data |>` pipeline) the formatter suggests one unit while `"aligned"` demands column 0. It also disables the aligned-argument/block-form tolerances on the specific lines an operator continuation covers.
  - `"either"` — accept both forms. Alignment is still rejected when the chain start itself sits below what the enclosing scope requires, so genuine under-indentation stays flagged.
