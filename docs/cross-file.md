...
Open-buffer authority is alias-aware for file URIs. If a buffer is opened through a case-only spelling difference (for example `child.R` for an on-disk `child.r`) or through a symlink path that resolves to a workspace file, Raven treats the corresponding graph URI as open: dependents revalidate as you type, cross-file scope reads the live buffer, and watched-file resyncs for the canonical spelling are skipped until close. Diagnostics still publish to the URI spelling the editor opened, and closed-file graph/index keys remain raw path spellings rather than symlink-canonicalized paths.
