...
For `[`, base subscripting is standard-eval, so indices are checked unless the indexed object is data.table-like: a known `data.table()` / `as.data.table()` / `fread()` object, or an unresolved object when data.table is detectably in play (a `library(data.table)` call, a `data.table::` reference, or a package `Imports:`/`importFrom`). Independently of that classification, a `[` call whose argument is a `:=` assignment (`d[, avg := mean(x), by = grp]`, `d[cond, tag := 0]`) is always treated as NSE and all of its indices are suppressed: base R has no `:=` function and no base `[` method evaluates one, so the call must be dispatching to a data.table-style method regardless of which package supplies it (other packages adopt this notation for their own containers). The `:=` must be the argument itself, not nested inside another expression, and it wins even over a known non-data.table object — `df[, y := 1]` on a plain data.frame is a runtime error either way, and an undefined-variable report on `y` would be misleading. `[[` is always checked — `DT[[x]]` references `x` as a real variable.
