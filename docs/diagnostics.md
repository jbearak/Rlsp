...
Editor diagnostics are live-document scoped. Indexed or sourced files contribute symbols and dependency information to an open document's analysis, but Raven does not publish their own Problems entries until the editor opens them. Closing a document clears its published diagnostics from the Problems panel.
