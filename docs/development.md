...
- A missing file prepares a `ClosedRemove` mutation through the same seam. The watched-files path combines CREATED/CHANGED upserts and DELETED removals with one complete package projection in `PreparedWatchedBatchAnalysis`. Exact watched generations, the full package basis, every open-record token, and the frozen disk projection are checked before the central commit, so a rejection changes neither closed-file nor package state.
