...
Raven also provides lightweight support for **JAGS** (`.jags`, `.bugs`) and **Stan** (`.stan`) files: [syntax highlighting](docs/syntax-highlighting.md#jags-and-stan), [completions](docs/completion.md#jags-and-stan) (keywords, distributions, versioned built-in callables, file-local symbols), [JAGS hover](docs/hover.md#jags-hover), [Stan hover](docs/hover.md#stan-hover), static signature help for JAGS built-ins, [JAGS and Stan syntax diagnostics](docs/diagnostics.md#jags-and-stan-parse-errors), [go-to-definition](docs/go-to-definition.md#jags-and-stan), [find references](docs/find-references.md#jags-and-stan), and [document outline with model structure navigation](docs/document-outline.md#jags-and-stan-model-structure). Strict JAGS-dialect diagnostics apply to both `.jags` and `.bugs`, case-insensitively; singular `.bug` is not a supported extension ([#724](https://github.com/jbearak/raven/issues/724)).
