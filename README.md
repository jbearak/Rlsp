...
Raven also supports **JAGS** (`.jags`, `.bugs`, `.bug`) and **Stan** (`.stan`) files: [syntax highlighting](docs/syntax-highlighting.md#jags-and-stan), [completions](docs/completion.md#jags-and-stan) (keywords, distributions, versioned built-in callables, file-local symbols), [JAGS hover](docs/hover.md#jags-hover), [Stan hover](docs/hover.md#stan-hover), static signature help for JAGS built-ins, [JAGS and Stan syntax diagnostics](docs/diagnostics.md#jags-and-stan-parse-errors), conservative [Stan undeclared-variable diagnostics](docs/diagnostics.md#stan-undeclared-variables), [go-to-definition](docs/go-to-definition.md#jags-and-stan), [find references](docs/find-references.md#jags-and-stan), and [document outline with model structure navigation](docs/document-outline.md#jags-and-stan-model-structure). Native Stan/JAGS syntax findings share a configurable per-file limit (default 500; `0` for unlimited) in the editor and `raven check`; Stan semantic findings use a separate fixed 500-per-file bound. Files ending in `.jags`, `.bugs`, or `.bug` receive strict JAGS-dialect validation; this does not claim general OpenBUGS, WinBUGS, MultiBUGS, or NIMBLE compatibility.
