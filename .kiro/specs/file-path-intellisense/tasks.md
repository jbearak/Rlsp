...
- [-] 1. Set up module structure and core types
  - [-] 1.1 Create `crates/raven/src/file_path_intellisense.rs` with module structure and imports
  ...
  - [ ] 1.2 Define `FilePathContext` enum and `DirectiveType` enum
  ...
  - [ ] 1.3 Add module declaration to `main.rs` or `lib.rs`
    - Export the new module for use by handlers
    - _Requirements: Foundation_

- [ ] 2. Implement context detection for source() calls
  - [ ] 2.1 Implement `is_source_call_string_context()` using tree-sitter AST
...
- [ ] 7. Implement completion item creation
  ...
  - [ ]* 7.2 Write property test for directory completion trailing slash
    - **Property 9: Directory Completion Trailing Slash**
    - **Validates: Requirements 2.6**
  
  - [ ]* 7.3 Write property test for output path separator
    - **Property 11: Output Path Separator**
    - **Validates: Requirements 4.3**

- [ ] 8. Implement path resolution for completions
  - [ ] 8.1 Implement relative path resolution (including `../` prefixes)
  ...
  - [ ] 8.2 Implement workspace-root-relative paths for directives (starting with `/`)
    - Resolve `/path` relative to workspace root for LSP directives
    - _Requirements: 2.4_
  
  - [ ] 8.3 Implement absolute filesystem paths for source() calls (starting with `/`)
    - Resolve `/path` as absolute filesystem path for source() calls
    - _Requirements: 2.5_
  
  - [ ] 8.4 Implement backslash normalization (convert `\\` to `/`)
    - Normalize escaped backslashes before path resolution
    - _Requirements: 4.1, 4.2_
  
  - [ ]* 8.5 Write property test for path separator normalization
    - **Property 10: Path Separator Normalization**
    - **Validates: Requirements 4.1, 4.2**

- [ ] 9. Implement main completion function
  - [ ] 9.1 Implement `file_path_completions()` main function
  ...
  - [ ]* 9.2 Write property test for workspace boundary enforcement
    - **Property 17: Workspace Boundary Enforcement**
    - **Validates: Requirements 7.2**

- [ ] 10. Checkpoint - Completions implementation complete
  - Ensure all completion tests pass, ask the user if questions arise.

- [ ] 11. Implement go-to-definition for file paths
  - [ ] 11.1 Implement `extract_file_path_at_position()` to get full path string at cursor
  ...
  - [ ] 11.2 Implement `file_path_definition()` main function
  ...
  - [ ]* 11.3 Write property test for source call go-to-definition
    - **Property 12: Source Call Go-to-Definition**
    - **Validates: Requirements 5.1, 5.2, 5.4**
  
  - [ ]* 11.4 Write property test for missing file returns no definition
    - **Property 13: Missing File Returns No Definition**
    - **Validates: Requirements 5.3**
  
  - [ ]* 11.5 Write property test for backward directive go-to-definition
    - **Property 14: Backward Directive Go-to-Definition**
    - **Validates: Requirements 6.1, 6.2, 6.3**
  
  - [ ]* 11.6 Write property test for forward directive go-to-definition
    - **Property 15: Forward Directive Go-to-Definition**
    - **Validates: Requirements 6.4**
  
  - [ ]* 11.7 Write property test for backward directives ignore @lsp-cd
    - **Property 16: Backward Directives Ignore @lsp-cd**
    - **Validates: Requirements 6.5**

- [ ] 12. Integrate with LSP handlers
  - [ ] 12.1 Modify `handlers::completion()` to check for file path context
  ...
  - [ ] 12.2 Add trigger characters `/` and `"` to completion options in backend.rs
    - Register additional trigger characters for path navigation
    - _Requirements: 3.1, 3.2, 3.4_
  
  - [ ] 12.3 Modify `handlers::goto_definition()` to check for file path context
...
- [ ] 13. Implement edge case handling
  - [ ] 13.1 Handle empty workspace (return empty completions)
    - Return empty list when workspace has no R files
    - _Requirements: 7.1_
  
  - [ ] 13.2 Handle paths outside workspace (exclude from completions)
    - Check resolved paths against workspace root
    - Exclude paths that escape workspace boundary
    - _Requirements: 7.2_
  
  - [ ] 13.3 Handle invalid path characters gracefully
    - Catch filesystem errors for invalid characters
    - Return empty completions without throwing errors
    - _Requirements: 7.3_
  
  - [ ] 13.4 Handle empty file path (show current directory contents)
    - When path is empty, list files in current directory
    - _Requirements: 7.4_
  
  - [ ] 13.5 Handle paths with spaces in quoted strings
    - Correctly parse paths containing spaces
    - _Requirements: 7.5_
  
  - [ ]* 13.6 Write property test for invalid character handling
    - **Property 18: Invalid Character Handling**
    - **Validates: Requirements 7.3**
  
  - [ ]* 13.7 Write property test for space handling in paths
